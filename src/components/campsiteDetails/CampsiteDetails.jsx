...
export default function CampsiteDetails({details}) {
    ...
    const images = details.images ? details.images.map(image => CF + image) : [CF + 'default.jpg'];
    ...
    const renderStars = (rating) => {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= ratingNumber) {
                stars.push(<i key={i} className="fa fa-star star-filled" aria-hidden="true"></i>); // Filled star
            } else {
                stars.push(<i key={i} className="fa fa-star-o star-empty" aria-hidden="true"></i>); // Empty star
            }
        }
    ...
    const toggleFavourite = async () => {
        try {
            ...
            if (isAlreadyFavourite) {
                ...
                response = await axios.put(`http://localhost:8080/api/users/removeFavourites/${user._id}`, {
                    favorites: details._id,
                });
                setIsFavourite(!isFavourite);
            } else {
                // If it's not a favorite, we add it
                response = await axios.put(`http://localhost:8080/api/users/addFavourites/${user._id}`, {
                    favorites: details._id,
                });
                setIsFavourite(isFavourite);
            }
    ...
    return (
        <div className="campsite-details">
            <div className="campsite-details-container">
                ...
                <div className="campsite-details-text">
                    <div className="campsite-header">
                        ...
                        <button className={`favourite-btn ${isFavourite ? 'active' : ''}`} onClick={toggleFavourite}>
                            <i className={`fa ${isFavourite ? 'fa-heart' : 'fa-plus'}`} aria-hidden="true"></i>
                    ...
                    <div className="campsite-desc">
                        {details.desc}
                    </div>
                    <div className="campsite-detail-row">
                        <div className="campsite-detail-type">
                            Location:
                        </div>
            ...
            <div className="camper-details-center">
                ...
                <CampsiteRating reviewIds={details.reviews} userId={user._id} campsiteId={details._id}/>
            </div>
