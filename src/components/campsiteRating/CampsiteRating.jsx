  ...
  const submitReview = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        try {
          const updatedCampsite = await axios.put(
            `http://localhost:8080/api/campsites/addReview/${campsiteId}`,
            {
  ...
  const fetchReviewsAndUsers = async () => {
    ...
    try {
      ...
      const reviewsWithUserNames = reviews.map((review) => {
        ...
        return {
          ...review,
          reviewerName: user ? user.username : "Anonymous",
        };
      });
    ...
    try {
      const response = await axios.put(
        `http://localhost:8080/api/campsites/${campsiteId}`,
        {
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <form onSubmit={(e) => e.preventDefault()}>
                  <div className="mb-3">
                    <label htmlFor="review-rating" className="col-form-label">
                      Rating:
                    </label>
                    <select
