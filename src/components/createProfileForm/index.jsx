...
const createProfileSchema = Yup.object().shape({
    ...
    bio: Yup.string().min(2, "Too Short!").max(500, "Too Long!").required("Bio is required so people get to know you better"),
    age: Yup.number().min(1, "Too Short!").max(100, "Too Long!").required("Age is required"),
    experience: Yup.string().min(2, "Too Short!").max(500, "Too Long!")
...
function ProfileForm({initialValues = _initialValues}) {
    ...
    const handleImageChange = async (e) => {
        if (e.target.files[0]) {
            setSelectedImage({pictureFile: e.target.files[0], picturePath: URL.createObjectURL(e.target.files[0])})
        }
    };
                ...
                {({errors, values}) => (<Form>
                    ...
                    >
                        <img
                            ...
                            src={selectedImage.picturePath ? selectedImage.picturePath : "/assets/defaultpp.jpg"}
                            alt=""
                        />
                        <input
                            type="file"
                    ...
                    <button className="update-btn submit-btn" type="submit" disabled={isUpdatingProfile}>
                        Edit Profile
                    </button>
                </Form>)}
            </Formik>
